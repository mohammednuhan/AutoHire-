...
logger = logging.getLogger("autohire.scrapers.naukri")


class NaukriScraper(BoardScraper):
    board_name = "naukri"
    ...
    async def extract_job_detail(self, listing: RawJobListing) -> FullJobDetail:
        ...
        description = html_to_plain_text(_description_from_html(html)) or listing.description or listing.title
    ...
    def _search_url(self, role: str, location: str) -> str:
        ...
        return f"{self.base_url}/jobs-in-{location_slug}?k={keywords}"

    async def _extract_listings_from_page(self, page: Any) -> list[RawJobListing]:
        html = await page.content()
        initial_state = _extract_initial_state(html)
        listings = _listings_from_state(initial_state)
        if listings:
            return listings
        return await _listings_from_dom(page)
...
def _listings_from_state(state: dict[str, Any] | None) -> list[RawJobListing]:
    if not state:
        return []
    candidates: list[dict[str, Any]] = []
    _collect_job_like_dicts(state, candidates)
    listings: list[RawJobListing] = []
    for item in candidates:
        job_id = str(item.get("jobId") or item.get("job_id") or item.get("id") or "")
        title = normalize_space(str(item.get("title") or item.get("jobTitle") or ""))
        company = normalize_space(str(item.get("companyName") or item.get("company") or ""))
        url = str(item.get("jdURL") or item.get("jobUrl") or item.get("url") or "")
        if url and url.startswith("/"):
            url = f"https://www.naukri.com{url}"
        if not job_id or not title or not company or "naukri.com" not in url:
            continue
        ...
        if not location and isinstance(placeholders, list) and placeholders:
            ...
            if isinstance(last_placeholder, dict):
                location = normalize_space(str(last_placeholder.get("label") or ""))
        listings.append(
            RawJobListing(
                external_id=job_id[:500],
                title=title[:500],
                company=company[:255],
                url=url,
                location=location or None,
                work_type=infer_work_type(text),
                posted_at=parse_posted_at(str(item.get("footerPlaceholderLabel") or item.get("createdDate") or "")),
                salary_range=normalize_space(str(item.get("salary") or "")) or None,
                salary_min_inr=salary_min,
                salary_max_inr=salary_max,
                description=normalize_space(str(item.get("jobDescription") or item.get("description") or ""))[:1000],
                skills_required=extract_skills_from_text(text),
                experience_level=normalize_space(str(item.get("experience") or item.get("experienceText") or "")) or None,
                metadata=item,
            )
        )
    return listings[:15]
...
def _collect_job_like_dicts(value: Any, output: list[dict[str, Any]]) -> None:
    if isinstance(value, dict):
        keys = set(value)
        if {"title", "companyName"} <= keys or {"jobTitle", "company"} <= keys or "jobId" in keys:
            output.append(value)
        for child in value.values():
            _collect_job_like_dicts(child, output)
...
async def _listings_from_dom(page: Any) -> list[RawJobListing]:
    cards = await page.locator("article, .srp-jobtuple-wrapper, .jobTuple").all()
    listings: list[RawJobListing] = []
    for card in cards[:15]:
        try:
            anchor = card.locator("a[href*='naukri.com']").first()
        ...
        if not url or not title:
            continue
        company = _line_after_title(text, title)
        salary_min, salary_max = parse_salary_range_inr(text)
        listings.append(
            RawJobListing(
...
def _description_from_html(html: str) -> str:
    match = re.search(r"<section[^>]*(?:job-desc|description)[^>]*>(.*?)</section>", html, flags=re.DOTALL | re.IGNORECASE)
    if match:
        return match.group(1)
    return html


def _line_after_title(text: str, title: str) -> str:
